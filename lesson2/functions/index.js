...
async function addProduct(data, context) {
    if (!authorized(context.auth.token.email)) {
        ...
        throw new functions.https.HttpsError('permission-denied', 'Only admin may invoke this addProduct function');
    }
...
exports.cfn_getProductList = functions.https.onCall(async (data, context) => {
    if (!authorized(context.auth.token.email)) {
        if (Constants.DEV) console.log(e);
        throw new functions.https.HttpsError('permission-denied', 'Only admin may invoke this getProductList function');
    }
    try {
        let products = [];
...
exports.cfn_updateProductDoc = functions.https.onCall(async (data, context) => {
    ...
    if (!authorized(context.auth.token.email)) {
        if (Constants.DEV) console.log(e);
        throw new functions.https.HttpsError('permission-denied', 'Only admin may invoke this updateProductDoc function');
    } 
    try {
        await admin.firestore().collection(Constants.COLLECTION_NAMES.PRODUCTS)
